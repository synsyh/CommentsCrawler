/**
  * Created by sunning on 2017/3/27.
  */
...
class Crawler(startPage: String,
              ...
              onDataLoaded: (String, Int, Array[Byte], Map[String, String]) =>
                Any = (url: String, status: Int, data: Array[Byte], headers: Map[String, String]) => {
                println(s"下载 $url 完成")
              }) {
  ...
  def crawl {
    val htmlText=new String(get(startPage)._2)
    crawlPageLinks(startPage, htmlText)
    latch.await()
  }
